...
class Multask_Wrapper:
    def __init__(self, tasks, device, main_config, task_config, seed, loading_data=True):
        ...
        self.device = device1                                  # GPU device idx
        ...
        self.checkpoint_dir = os.path.join(checkpoint_dir_path, self.model_name)
        if not os.path.exists(self.checkpoint_dir):
            os.mkdir(self.checkpoint_dir)
        # folders preparation to save tensorboard and other logs
        self.tb_log_dir = os.path.realpath(os.path.join(tb_log_path, self.model_name))
        ...
        for i, task in enumerate(self.tasks):
            if task == 'COG' and self.config['COG']['type'] == 'reg':
                thres['NC'], thres['DE'] = COG_thresholding(
                    os.path.realpath(os.path.join(self.tb_log_dir, csv_name + '_eval.csv'))
                )
            elif task == 'ADD':
        ...
        with torch.no_grad():
            for stage in stages:
                ...
                dataloader = DataLoader(
                    Test_Data(os.path.realpath(os.path.join(self.csv_dir, stage + '.csv'))),
                    batch_size=1,
                    shuffle=False
                )
                for mri, name in dataloader:
                    case = {'filename': os.path.basename(name[0])}
                    ...
                    content.append(case)
                tb_log_csv_save_path = os.path.abspath(os.path.join(self.tb_log_dir, stage + '_eval.csv'))
                with open(tb_log_csv_save_path, 'w') as csvfile:
                ...
                if join:
                    ...
                    result.to_csv(
                        os.path.realpath(os.path.join(self.tb_log_dir, stage + '_eval.csv')),
                        index=False
                    )
            ...
            for stage in stages:
                ...
                for mri, name in dataloader:
                    ...
                    if layer == 'late':
                        ...
                        np.save(
                            os.path.abspath(os.path.join(late_embeddings_path, case['filename'])),
                            middle
                        )
                    elif layer == 'mid':
                        #print("len.self.MLPS= ", len(self.MLPs))
                        ...
                        #print(middle)
                        np.save(
                            os.path.abspath(os.path.join(mid_embeddings_path, case['filename'])),
                            middle
                        )
                    elif layer == 'early':
                        ...
                        #print(middle.shape)
                        np.save(
                            os.path.abspath(os.path.join(early_embeddings_path, case['filename'])),
                            middle
                        )
                    #else:
                        #print('bad')
    ...
    def shap_mid(self, task_idx=1, path=data_2_sq_path, file='test.csv', background_idx=1, layer='block2conv'):
    ...
    def valid_an_epoch(self, metric='AUC', stage='valid'):
        self.gen_score((stage, ), load_weight=False)  # using default thres value to generate score for validation set
        if metric == 'MCC':
            thres = self.get_optimal_thres(stage)       # get optimal thres for validation
            self.gen_score((stage, ), thres, load_weight=False) # apply the optimal thres on validation to generate score and pred
            metric = perform_table([os.path.realpath(os.path.join(self.tb_log_dir, '{}_eval.csv'.format(stage)))])
        elif metric == 'AUC':
            metric = ROC_PR_curves([os.path.realpath(os.path.join(self.tb_log_dir, '{}_eval.csv'.format(stage)))], stage)
        return metric
    ...
    def saveWeights(self, clean_previous=True):
        if clean_previous:
            files = glob(os.path.abspath(os.path.join(self.checkpoint_dir, '*.pth')))
            for f in files:
                os.remove(f)
        torch.save(
            self.backbone.state_dict(),
            os.path.abspath(os.path.join(self.checkpoint_dir, 'backbone_{}.pth'.format(self.train_step)))
        )
        for task, model in zip(self.tasks, self.MLPs):
            torch.save(
                model.state_dict(),
                os.path.abspath(os.path.join(self.checkpoint_dir, '{}_{}.pth'.format(task, self.train_step)))
            )
    ...
    def load_backbone_Weights(self, weight_path=None):
        if weight_path is None:
            paths = glob(os.path.abspath(os.path.join(self.checkpoint_dir, 'backbone*.pth')))
            last_index = max(map(lambda p: int(os.path.basename(p)[9:-4]), paths))
            target_file = os.path.realpath(os.path.join(
                self.checkpoint_dir,
                'backbone_{}.pth'.format(last_index)
            ))
        else:
            target_file = weight_path
        print('loading ', target_file)
        # weights = torch.load(target_file, map_location=lambda storage, loc: storage.cuda(self.device))
        weights = torch.load(target_file, map_location='cuda:0')
        try:
            self.backbone.load_state_dict(weights)
        except:
            self.backbone.load_state_dict(remove_module(weights))

    def load_MLP_weights(self, weight_paths=None):
        for i, task in enumerate(self.tasks):
            if weight_paths is None:
                paths = glob(os.path.abspath(os.path.join(self.checkpoint_dir, task + '*.pth')))
                task_strlen = len(task) + 1
                last_index = max(map(lambda p: int(os.path.basename(p)[task_strlen:-4]), paths))
                target_file = os.path.realpath(os.path.join(
                    self.checkpoint_dir,
                    '{}_{}.pth'.format(task, last_index)
                ))
            else:
                target_file = weight_paths[i]
            print('loading ', target_file)
            try:
                # self.MLPs[i].load_state_dict(torch.load(target_file, map_location='cuda:0'))
                self.MLPs[i].load_state_dict(torch.load(target_file, map_location='cuda:0'))
