...
class TaskData(Dataset):
    ...
    def __init__(self,
        ...
        self.Data_list, self.Labels_list, self.dataset_name_list = read_task_csv(
            os.path.realpath(os.path.join(csv_dir, '{}.csv'.format(stage))),
            Task,
            task_config
        )
        print("self.Lable_list.len= ", len(self.Labels_list))
        # print("self.dataset_name_list= ", self.dataset_name_list)
    ...
    def __getitem__(self, idx):
        ...
        # 把.nii扩展名替换为.npy
        dp, fn = os.path.split(self.Data_list[idx])
        i = fn.rfind('.')
        if i >= 0 and fn[i + 1:] == 'nii':
            fn = fn[:i + 1] + 'npy'
        fp = os.path.realpath(os.path.join(dp, fn))
        ...
        if self.patch == 'random':
            ...
            if self.trans:
                data = self.trans.apply(data).astype(np.float32)
    ...
    def get_sample_weights(self, ratio={}):
        ...
        #print("weight self.Labels_list= ", self.Labels_list)
        #print("weight self.dataset_name_list= ", self.dataset_name_list)
...
def read_task_csv(filename, task, config):
    #print("read_task_csv filename= ", filename)
    #print("read_task_csv task= ", task)
    ...
    with open(filename, 'r') as csvfile:
        ...
        for row in reader:
            if not row[task]:
                continue
            mri_list.append(os.path.realpath(os.path.join(mri_path, row['filename'])))
...
def read_filenames(filename):
    ...
    with open(filename, 'r') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            data_list.append(os.path.realpath(os.path.join(mri_path, row['filename'])))
    return data_list
