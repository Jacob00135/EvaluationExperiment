...
from config import tb_log_path
...
def crossValid_cm(csv_files, stage):
    ...
    fig.savefig(
        os.path.realpath(os.path.join(os.path.dirname(csv_files[0]), stage + '_cm3.jpg'))
    )
    print(cm)
    print('---------------------------------')
...
def ROC_PR_curves(csv_files, stage):
    ...
    nc_roc =  generate_roc(
        ...
        'NC',
        'g',
        os.path.abspath(os.path.join(dir_path, stage + '_NC_roc.jpg'))
    )
    de_roc =  generate_roc(
        csv_files,
        'DE',
        'darkorange',
        os.path.abspath(os.path.join(dir_path, stage + '_DE_roc.jpg'))
    )
    ...
    nc_pr =  generate_pr(
        ...
        'NC',
        'g',
        os.path.abspath(os.path.join(dir_path, stage + '_NC_pr.jpg'))
    )
    de_pr =  generate_pr(
        csv_files,
        'DE',
        'darkorange',
        os.path.abspath(os.path.join(dir_path, stage + '_DE_pr.jpg'))
    )
...
def perform_table(csv_files, output_name):
    ...
    for metric in ['Accuracy', 'F-1', 'Sensitivity', 'Specificity', 'MCC']:
        row = [metric]
        cache[metric] = {}
        for task in ['NC', 'MCI', 'DE', 'macro-3ways']:
...
def whole_eval_package(model_name, stage, performTableFileName):
    ...
    csv_files = [os.path.realpath(os.path.join(tb_log_path, model_name, '{}_eval.csv'.format(stage)))]
