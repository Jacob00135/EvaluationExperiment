...
class TableData:
    def __init__(self):
        ...
        self.imageDir = '/root/workspace/lxs/ADNI/npy/'    #'../data/datasets/ADNIGO/'
        self.imageFileNameList, self.RIDList, self.Visitlist = self.get_filenames_and_IDs(self.imageDir)
        self.columnNames = []
    ...
    def get_filenames_and_IDs(self, path):
        ...
        nature_imginfo = pd.read_csv(
            '/root/workspace/lxs/ncomms2022-main/ncomms2022-main/lookupcsv/dataset_table/sc_nature_imginfo.csv')
        fileNameList = []
        Visitlist = []
        for index, row in nature_imginfo.iterrows():

            if str(row['Phase']) == 'ADNI GO':
                print(str(row['Phase']))
                fileNameList.append(row['filename'])
                Visitlist.append(row['VISCODE'])
        ...
        return fileNameList, RIDList, Visitlist
    ...
    def addColumns_path_filename(self):
        ...
        for idx, id in enumerate(self.RIDList):
            ...
            self.content[id]['filename'] = self.imageFileNameList[idx].replace('.npy', '.npy')
    ...
    def addColumns_mmse(self):
        ...
        old_variables = ['mmse', 'Orientation', 'Working Memory', 'Concentration', 'Memory Recall', 'Language',
                         'Visuospatial']
        variables = ['mmse']
        old_variables = ['mmse']
        self.columnNames.extend(variables)
    ...
    def addColumns_nb(self):
        variables = ['nb_CLOCKSCOR', 'nb_COPYSCOR', 'nb_AVTOTB', 'nb_CATANIMSC', 'nb_TRAASCOR', 'nb_TRABSCOR',
                     'nb_BNTTOTAL', 'nb_AVDEL30MIN', 'nb_AVDELTOT', 'nb_ANARTERR']
        old_variables = ['CLOCKSCOR', 'COPYSCOR', 'AVTOTB', 'CATANIMSC', 'TRAASCOR', 'TRABSCOR', 'BNTTOTAL',
                         'AVDEL30MIN', 'AVDELTOT', 'ANARTERR']
        self.columnNames.extend(variables)
    ...
    def addColumns_moca(self):
        variables = ['moca', 'moca_execu', 'moca_visuo', 'moca_name', 'moca_atten', 'moca_senrep', 'moca_verba',
                     'moca_abstr', 'moca_delrec', 'moca_orient']
        old_variables = ['moca', 'moca_execu', 'moca_visuo', 'moca_name', 'moca_atten', 'moca_senrep', 'moca_verba',
                         'moca_abstr', 'moca_delrec', 'moca_orient']
        variables = ['moca']
        old_variables = ['moca']
        self.columnNames.extend(variables)
    ...
    def addColumns_adas(self):
        variables = ['adas_Q1score', 'adas_Q2score', 'adas_Q3score', 'adas_Q4score', 'adas_Q5score', 'adas_Q6score',
                     'adas_Q7score', 'adas_Q8score', 'adas_Q9score', 'adas_Q10score', 'adas_Q11score', 'adas_Q12score',
                     'adas_Q13score', 'adas_TOTAL13', 'adas_TOTAL_Score']
    ...
    def writeTable(self):
        ...
        with open(self.datasetName + '.csv', 'w', encoding='UTF-8') as csvfile:
